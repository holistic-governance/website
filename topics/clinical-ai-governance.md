...
### The CDSS exemption no longer covers AI-enabled tools

This is the change most likely to catch providers out. The CDSS exemption in the *Therapeutic Goods (Medical Devices) Regulations 2002* (Schedule 4, Part 2) requires **all three** criteria to be met: the sole purpose is providing or supporting a recommendation to a health professional; the software does not directly process or analyse a medical image or signal from another device; and the software does not replace the clinical judgement of a health professional.

The TGA rewrote this guidance on **7 October 2025**, confirming that AI/black-box CDSS cannot be exempt. The current guidance (last updated 29 January 2026) states: *"With the rapid uptake of artificial intelligence and its growing incorporation into medical devices, it is important to understand that an AI-enabled CDSS will not meet the exemption criteria."*

The reason sits in the third criterion. To satisfy it, the TGA says software must "be transparent in how it generates recommendations (i.e. cannot use proprietary analysis or AI to generate recommendations)", must let the health professional easily understand and verify the recommendation's accuracy, and must clearly reference the logic, clinical guidelines, process or evidence each recommendation rests on. Software providing information the clinician "cannot independently verify, or information that is not available other than through the software", fails the criterion. The TGA contrasts a transparent "glass box" decision-tree approach with opaque integrated methods.

The practical effect: a rules-based alert that cites its source may be exempt, while the same alert generated by a model is not exempt and requires ARTG inclusion. Two governance consequences follow:

- **Exempt does not mean unregulated.** Sponsors of exempt CDSS must still notify the TGA within 30 working days of supply via the CDSS Exemption Notification Form, comply with the Essential Principles, meet advertising requirements and report adverse events, and remain subject to TGA recall and hazard-alert powers.
- **An existing exemption is not durable.** If a vendor adds AI capability to a tool you hold an exemption basis for, that basis may no longer hold. This is the single most likely way a provider's documented device-boundary position goes quietly out of date.
...
2. **Device boundary** — Which of these tools have a therapeutic intended purpose, and for each, what evidence do we hold: ARTG inclusion for that intended use, or a documented basis for exclusion or exemption? Where a basis rests on the clinical decision support exemption, when was it last tested against the TGA's position that an AI-enabled CDSS cannot meet the exemption criteria? Where a tool is a regulated device, who owns our parallel TGA adverse-event reporting obligation?
...
- TGA — Understanding clinical decision support system software regulation (updated 29 January 2026): https://www.tga.gov.au/resources/guidance/understanding-clinical-decision-support-system-software-regulation
- TGA — Exempt software as a medical device: https://www.tga.gov.au/products/medical-devices/software-and-artificial-intelligence-ai/overview/exempt-software-medical-device
