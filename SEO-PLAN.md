...
- [ ] Set up query tracking: in GSC Performance, save a regex filter such as `board report|board assurance|aged care governance` and record baseline impressions/clicks in this doc.
- [ ] After deploy: Request Indexing for `/aged-care-board-reporting.html`, `/` (homepage), `/board-reporting-health-check.html` (newly added to sitemap), and the two updated articles.
- [ ] After deploy: IndexNow ping for the same URLs + `sitemap.xml`, `llms.txt`, `llms-full.txt` (template below; use `curl` on macOS, not `curl.exe`).
