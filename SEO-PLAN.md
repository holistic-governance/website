...
**Board reporting / board assurance push (2026-07-26).** AI-retrieval testing showed hg-au.com surfaces for "clinical governance consultants" queries but not for "aged care governance", "board reporting" or "board assurance" framings (competitors ranking: Ageing Australia, GovernWith, StewartBrown, DHD Consultancy). On-page/schema/llms coverage and a dedicated landing page (`aged-care-board-reporting.html`) shipped 2026-07-26 — see commits log. Remaining manual steps:

- [ ] **Validate the retrieval gap in Search Console** before attributing movement to the on-page changes — check Performance → Queries for impressions on "board reporting", "board assurance", "aged care governance" variants now (baseline) and again at +6–8 weeks. The observed gap is AI/web retrieval, not a confirmed SERP position; cause may be authority/backlinks rather than on-page.
- [ ] Set up query tracking: in GSC Performance, save a regex filter such as `board report|board assurance|aged care governance|ai governance|ai vendor|prototype` and record baseline impressions/clicks in this doc (covers the board-reporting push and the ai-technology / app-development push).
- [x] **Request Indexing day 1 — submitted 2026-07-26** (via URL Inspection, domain property `sc-domain:hg-au.com`): `/ai-technology.html`, `/app-development.html`, `/aged-care-board-reporting.html`, `/articles/ai-governance-aged-care.html`, `/board-reporting-health-check.html`, `/` (homepage), `/resources.html`. IndexNow pings for both change sets also submitted 2026-07-26 — HTTP 200.
- [ ] **Request Indexing day 2 — 2026-07-27** (rate limit ~10–12/day): `/articles/reform-without-reporting.html`, `/articles/liquidity-standard-targeted-review.html`, `/articles/asd-post-quantum-vendor-questions.html`, `/articles/data-quality-foundation-of-ai.html`, `/reporting-dashboards.html`. Paste each plain URL into the GSC top search bar → Request Indexing.
...
| 4 | 23 Jun 2026 | AI Governance for Aged Care: A Provider's Practical Framework | "AI governance aged care" | 2,000 | ✅ Published 26 Jul 2026 (`articles/ai-governance-aged-care.html`, category "AI Governance" → related-solutions links ai-technology) |
...
| `ai-governance-aged-care.md` | AI governance for aged care providers — no AI-specific statute; obligations via Aged Care Act 2024 (ss 179/180), Strengthened Standards, Privacy Act, TGA SaMD boundary; voluntary frameworks (AI Ethics Principles, ISO/IEC 42001, National Model CGF); eight governance components; common uses and gaps (added 2026-07-26) | 1,150 |
...
| `787b6bb` | **2026-07-24** — Add SANS Security Awareness & Culture Maturity Model card to ai-technology.html frameworks grid; IndexNow ping HTTP 200. |
| *(2026-07-24)* | **Security awareness lead-magnet funnel** — new gate page `security-awareness-dashboard.html` (mirrors board-reporting-health-check.html: Name/Work Email/Organisation required, posts `enquiryType: security-leadmagnet` to Cloud Run dashboard, `lead_magnet_download` GA event + AW tag, then reveals download). Deliverable at `downloads/hg-security-awareness-board-dashboard.html` with noindex meta; `Disallow: /downloads/` added to all robots.txt blocks; deliverable excluded from sitemap/llms. Entry points: Free Resource band on ai-technology.html + new "Free Tools" section on resources.html (also surfaces the Board Reporting Health Check, previously only linked from reporting-dashboards.html). Gate page added to sitemap (0.7) + llms.txt. Endpoint verified with labelled test lead (success, email sent). |
| `125f8b2` | **2026-07-24** — Four-agent review (security/audit/compliance/improvement) actioned: XSS escaping in dashboard tool, privacy policy corrections (resource-download collection, Resend USA disclosure, Ads tag, NDB timeline), SANS non-affiliation + licence-in-file, gate-page screenshot + dedicated OG image (og-security-awareness.png), GA4 generate_lead/lead_capture_failed/file_download events on both funnel pages, homepage Free Tools strip, PQC-article promo, Free Tools ranked above premium on resources. Backend fixes (email validation, factual delivery email per Spam Act Sch 1, 24h delivery cap, no resend.dev fallback) on hg-au branch `funnel-compliance-fixes` pending deploy. Note: Google Ads conversion still needs a conversion label (send_to) created in the Ads UI. |
| *(2026-07-26 b)* | **AI-technology / app-development discoverability push** — WebPage+Speakable+audience schema added to both pages; app-development title/meta/OG rewritten with search phrasing ("proof of concept", "prototype development") replacing the circular description; internal links: reporting-dashboards→app-development, PQC + data-quality articles→ai-technology (via .md, rebuilt), security-dashboard entry link restored on ai-technology (lost in Version B redesign); build-articles.js stale "archived pages" comment removed and new "AI Governance" category mapped to ai-technology; **content calendar #4 published** (ai-governance-aged-care) + new `/topics/ai-governance-aged-care.md` dossier (10th); both registered in resources.html, sitemap, llms.txt, llms-full.txt. Article rebuild ran cleanly; liquidity hand-polish restored via git checkout; liquidity + reform .md now carry `dateModified` frontmatter so future rebuilds keep schema dates. |
| *(2026-07-26)* | **Board reporting / board assurance discoverability** — homepage title/meta/OG/hero/service-card copy now carries "board reporting", "board assurance", "aged care governance"; ProfessionalService schema extended (knowsAbout + hasOfferCatalog with serviceType/areaServed per service). New landing page `aged-care-board-reporting.html` (Service + WebPage/Speakable + FAQPage + BreadcrumbList schema; converts via Board Reporting Health Check; links to dashboards, healthcare, articles). llms.txt + llms-full.txt mirror the new terms and register the page. Internal links added from homepage (service card 01 + Free Tools card) and from "Reform Without Reporting" + "Liquidity Standard" articles (edited in both .md and .html to avoid a full rebuild clobbering the liquidity hand-polish; dateModified bumped). Sitemap: added landing page (0.9) + board-reporting-health-check.html (0.7, previously missing), bumped lastmods. |
